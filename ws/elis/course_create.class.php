...
class local_datahub_elis_course_create extends external_api {
    ...
    public static function course_create(array $data) {
        ...
        $customfields = self::get_course_custom_fields();
        ...
        if (!empty($errors)) {
            rlip_importplugin_version1elis::log_customfield_errors($errors, $context, $course->id, get_class());
            $msg = get_string('ws_course_create_partial_success_msg', 'local_datahub');
        } else {
            $msg = get_string('ws_course_create_success_msg', 'local_datahub');
        }

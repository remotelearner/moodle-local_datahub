...
class local_datahub_elis_course_update extends external_api {
    ...
    public static function course_update(array $data) {
        ...
        if (!empty($errors)) {
            rlip_importplugin_version1elis::log_customfield_errors($errors, $context, $crsid, get_class());
            $msg = get_string('ws_course_update_partial_success_msg', 'local_datahub');
        } else {
            $msg = get_string('ws_course_update_success_msg', 'local_datahub');
        }

...
class local_datahub_elis_program_create extends external_api {
    ...
    public static function program_create(array $data) {
        ...
        $customfields = self::get_program_custom_fields();
        ...
        if (!empty($errors)) {
            rlip_importplugin_version1elis::log_customfield_errors($errors, $context, $prg->id, get_class());
            $msg = get_string('ws_program_create_partial_success_msg', 'local_datahub');
        } else {
            $msg = get_string('ws_program_create_success_msg', 'local_datahub');
        }

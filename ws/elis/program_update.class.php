...
class local_datahub_elis_program_update extends external_api {
    ...
    public static function program_update(array $data) {
        ...
        if (!empty($errors)) {
            rlip_importplugin_version1elis::log_customfield_errors($errors, $context, $curid, get_class());
            $msg = get_string('ws_program_update_partial_success_msg', 'local_datahub');
        } else {
            $msg = get_string('ws_program_update_success_msg', 'local_datahub');
        }

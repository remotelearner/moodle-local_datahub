...
class local_datahub_elis_userset_create extends external_api {
    ...
    public static function userset_create(array $data) {
        ...
        if (!empty($errors)) {
            rlip_importplugin_version1elis::log_customfield_errors($errors, $context, $userset->id, get_class());
            $msg = get_string('ws_userset_create_partial_success_msg', 'local_datahub');
        } else {
            $msg = get_string('ws_userset_create_success_msg', 'local_datahub');
        }
